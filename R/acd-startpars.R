...
.meqstartpars = function(pars, arglist) {
  ...
  # In case we just have simple arma mean equation
  if(sum(modelinc[4:6]) == 0 ){
   ...
   tmph = ewmav(scale(data,scale = FALSE));
  y = data
  fit.mean = lm(y ~ tmph)
  pars[idx["mu", 1]:idx["mu", 2], 1] = fit.mean$coef["(Intercept)"]
  pars[idx["archm",1]:idx["archm",2],1] = fit.mean$coef["tmph"]
  if(modelinc[5]==1){
    pars[idx["skm",1]:idx["skm",2],1] = 0;
  }
  if(modelinc[6] == 1){
    pars[idx["shm",1]:idx["shm",2],1] = 0;
  }
    }
  arglist$tmph = tmph
...
.meqstart = function(pars, arglist) {
  ...
  if(modelinc[6]>0){
    pars[idx["skm", 1]:idx["skm", 2], 1] = 0
    pars[idx["skm", 1]:idx["skm", 2], 5] = -10
    pars[idx["skm", 1]:idx["skm", 2], 6] =  10
    if(!is.null(start.pars$skm)) pars[idx["skm", 1]:idx["skm", 2], 1] = start.pars$skm[1]
    if(any(!is.na(match(fixed.names, "skm")))){
      pars[idx["skm", 1]:idx["skm", 2], 1] = as.numeric(fixed.pars$skm)
      pars[idx["skm", 1]:idx["skm", 2], 5] = fixed.pars$skm
      pars[idx["skm", 1]:idx["skm", 2], 6] = fixed.pars$skm
    }
  }
  #shm
  if(modelinc[7]>0){
    pars[idx["shm", 1]:idx["shm", 2], 1] = 0
    pars[idx["shm", 1]:idx["shm", 2], 5] = -10
    pars[idx["shm", 1]:idx["shm", 2], 6] =  10
    if(!is.null(start.pars$shm)) pars[idx["shm", 1]:idx["shm", 2], 1] = start.pars$shm[1]
    if(any(!is.na(match(fixed.names, "shm")))){
      pars[idx["shm", 1]:idx["shm", 2], 1] = as.numeric(fixed.pars$shm)
      pars[idx["shm", 1]:idx["shm", 2], 5] = fixed.pars$shm
      pars[idx["shm", 1]:idx["shm", 2], 6] = fixed.pars$shm
    }
  }
  return(pars)
}
  ...
  if (modelinc[19] > 0) {
    ...
    if (modelinc[22] > 0) {
      if (is.na(pars[idx["sh1beta", 1]:idx["sh1beta", 2], 5]))
        pars[idx["sh1beta", 1]:idx["sh1beta", 2], 5] = xshape1$shapepar.LB[3]
      if (is.na(pars[idx["sh1beta", 1]:idx["sh1beta", 2], 6]))
        pars[idx["sh1beta", 1]:idx["sh1beta", 2], 6] = xshape1$shapepar.UB[3]
  ...
  if (modelinc[24] > 0) {
      ...
      if (modelinc[25] > 0) {
        if (is.na(pars[idx["sh2alpha", 1]:idx["sh2alpha", 2], 5]))
          pars[idx["sh2alpha", 1]:idx["sh2alpha", 2], 5] = xshape1$shapepar.LB[2]
        if (is.na(pars[idx["sh2alpha", 1]:idx["sh2alpha", 2], 6]))
          pars[idx["sh2alpha", 1]:idx["sh2alpha", 2], 6] = xshape1$shapepar.UB[2]
        if (is.null(start.pars$sh2alpha))
          pars[idx["sh2alpha", 1]:idx["sh2alpha", 2], 1] = xshape1$shapepars[2] else pars[idx["sh2alpha", 1]:idx["sh2alpha", 2], 1] = start.pars$sh2alpha[1]
