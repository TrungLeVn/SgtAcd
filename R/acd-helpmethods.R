...
.acdsetbounds = function(object, value){
  ...
  if(model$modelinc[10]>0 && any(parnames=="skew")){
  ...
  if(model$modelinc[14]>0 && any(parnames=="shape")){
...
.acdfitted = function(object)
{
  if(class(object)[1] == "ACDfit" | class(object)[1] == "ACDfilter"){
    D = object@model$modeldata$index[1:length(object@model$modeldata$data)]
  }
...
.acdshape = function(object, transformed = TRUE)
{
  ...
  if(is(object, "ACDfit")){
    ...
    if(transformed){
      ...
      ans = apply(object@forecast$tshapeFor, 2, function(x) exptransform(x, object@model$sbounds[3], object@model$sbounds[5], inverse = TRUE))
    }
  } else if(is(object, "ACDpath")){
    if(transformed){
      ...
      ans = apply(object@path$shapeSim, 2, function(x) exptransform(x, object@model$sbounds[3], object@model$sbounds[5], inverse = TRUE))
      rownames(ans) = paste("T+",1:NROW(object@path$shapeSim), sep="")
    }
  }
  else if(is(object, "ACDsim")){
    if(transformed){
      ...
      ans = apply(object@simulation$shapeSim, 2, function(x) exptransform(x, object@model$sbounds[3], object@model$sbounds[5], inverse = TRUE))
      rownames(ans) = paste("T+",1:NROW(object@simulation$shapeSim), sep="")
    }
  } else if(is(object, "ACDroll")){
    if(transformed){
...
.acdinfocriteria = function(object)
{
  if(is(object, "ACDfilter")){
    ...
    garchnp = sum(object@fit$ipars[1:10,4])
...
setMethod("show",
          ...
          function(object){
            ...
            cat(paste("\n-----------------------------------", sep = ""))
            cat(paste("\nGARCH Model\t: ", vmodel,"(", modelinc[5], ",", modelinc[6], ")\n", sep=""))
            if(vmodel == "fGARCH"){
            ...
            if(sum(object@model$dmodel$skewOrder)>0){
              ...
              cat(paste("\n-----------------------------------", sep = ""))
              cat(paste("\nACD Skew Model\t: ", object@model$dmodel$skewmodel,"(", modelinc[11], ",", modelinc[12], ",", modelinc[13],")", sep=""))
            ...
            if(sum(object@model$dmodel$shapeOrder)>0){
              ...
              cat(paste("\n-----------------------------------", sep = ""))
              cat(paste("\nACD Shape Model\t: ", object@model$dmodel$shapemodel,"(", modelinc[15], ",", modelinc[16], ",", modelinc[17],")", sep=""))
...
setMethod("show",
          ...
          function(object){
            ...
            if(object@fit$convergence == 0){
              ...
              cat(paste("\n------------------------------------\n",sep=""))
              tmp2 = rugarch:::.weightedBoxTest(stdresid, p = 2, df = sum(modelinc[5:6]))
              print(tmp2, digits = 4)
              cat(paste("d.o.f=", sum(modelinc[5:6]), sep=""))
              cat("\n\nWeighted ARCH LM Tests")
              cat(paste("\n------------------------------------\n",sep=""))
              gdf = sum(modelinc[5:6])
          ...
          function(object){
            if(!is.null(object@model$noncidx)){
              ...
              if(sum(model$dmodel$skewOrder)>0){
                cat(paste("\nACD Skew Model\t: ", model$dmodel$skewmodel,"(", modelinc[11], ",", modelinc[12], ",", modelinc[13],")", sep=""))
              }
              if(sum(model$dmodel$shapeOrder)>0){
                cat(paste("\nACD Shape Model\t: ", model$dmodel$shapemodel,"(", modelinc[15], ",", modelinc[16], ",", modelinc[17],")", sep=""))
              }
