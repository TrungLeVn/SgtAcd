...
##   the Free Software Foundation, either version 3 of the License, wo
		...
		control, LB, UB, cluster, arglist,rseed)
{
	if(arglist$fit.control$n.sim>0){
		...
		truncate_fun = function(x){
           return(list(mean = unname(x),sd = 2*abs(unname(x))))
		}
		#dopt = lapply(pars, function(x) truncate_fun(x))
		dopt = list()
...
.ucminfsolver = function(pars, fun, gr, hessian, parscale, control, LB, UB, arglist){
	...
	arglist$UB = UB
	arglist$transform = TRUE
	ans = ucminf::ucminf(fn = fun, gr = NULL, par = .invlogtransform(pars, LB,UB), arglist = arglist,
...
.optimsolver = function(pars, fun, gr, control, LB, UB, arglist){
	...
	#ans = optim(fn = fun, gr = NULL, par = pars, arglist = arglist,
	 #           	control = control, method = method)
#ans = optim(fn = fun,gr = NULL,par = pars,arglist = arglist,lower = LB, upper = UB,control = control,method = "L-BFGS-B",lower = LB, upper = UB)
if (inherits(ans, "try-error")) {
		sol = list()
		sol$convergence = 1
...
.ucminf.ctrl = function(control){
	...
	if(is.null(control$maxeval)) control$maxeval = 2000
