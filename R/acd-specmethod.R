...
.acdspec <- function(variance.model = list(model = "gjrGARCH", garchOrder = c(1, 1), variance.targeting = FALSE),
  ...
  modelinc = rep(0, 35)
  names(modelinc) = c("mu", "ar", "ma", "archm", "skm", "shm",
  ...
  #----
  mm = match(names(mean.model), c("armaOrder", "include.mean","archm","skm","shm"))
  if (any(is.na(mm))) {
    idx = which(is.na(mm))
    enx = NULL
  ...
  #-------
  dmodel = list(model = "sgt", skewOrder = c(1, 1, 1), skewshock = 1, skewshocktype = 1, skewmodel = "pwl", volsk = FALSE,
                shape1Order = c(1 ,1, 1), shape1shock = 1, shape1shocktype = 1, shape1model = "pwl", volsh1 = FALSE,
                shape2Order = c(1 ,1, 1), shape2shock = 1, shape2shocktype = 1, shape2model = "pwl", volsh2 = FALSE, exp.rate = 1)
...
if(dmodel$model == "sst"){
  ...
  rownames(pos.matrix) = c("mu", "ar", "ma", "archm", "skm", "shm",
                           "omega", "alpha", "beta", "gamma",
                           "skew", "shape1","shape2",
                           "skcons", "skalpha", "skgamma", "skbeta", "volsk",
                           "sh1cons", "sh1alpha", "sh1gamma", "sh1beta","volsh1",
  ...
  mm = mm - length(which(modelinc[c(2,3,8,9,10,15,16,17,20,21,22,25,26,27)] > 0))
  ...
  rownames(pidx) = c("mu", "ar", "ma", "archm", "skm", "shm",
                     "omega", "alpha", "beta", "gamma",
                     "skew", "shape1","shape2",
                     "skcons", "skalpha", "skgamma", "skbeta", "volsk",
                     "sh1cons", "sh1alpha", "sh1gamma", "sh1beta","volsh1",
