...
.expand.model <- function(model) {
  ...
  for (i in 1:22) {
    if (model[i] > 0) {
      if (any(c(2, 3, 5, 6, 7, 12, 13, 14, 16, 17, 18, 20,21,22) == i)) {
...
.acdspec <- function(variance.model = list(model = "gjrGARCH", garchOrder = c(1, 1), variance.targeting = FALSE),
  ...
  modelinc = rep(0, 31)
  names(modelinc) = c("mu", "ar", "ma",
                      "omega", "alpha", "beta", "gamma",
                      "skew", "shape1", "shape2",
                      "skcons", "skalpha", "skgamma", "skbeta",
                      "sh1cons", "sh1alpha", "sh1gamma", "sh1beta",
                      "sh2cons", "sh2alpha", "sh2gamma", "sh2beta",
  ...
  #----
  mm = match(names(mean.model), c("armaOrder", "include.mean"))
  if (any(is.na(mm))) {
    idx = which(is.na(mm))
    enx = NULL
  ...
  dm = match(names(distribution.model), c("model",
                                          ...
                                          "shape1Order", "shape1shock","shape1shocktype", "shape1model",
                                          "shape2Order", "shape2shock", "shape2shocktype", "shape2model",
                                          "exp.rate"))
  if (any(is.na(dm))) {
    idx = which(is.na(dm))
    enx = NULL
  ...
  if (is.null(vmodel$variance.targeting))
    modelinc[4] = 1 else modelinc[4] = as.integer(1 - vmodel$variance.targeting)
  #------
  # Specify the mean equation. The undefined argument will goes with default
  #------
  mmodel = list(armaOrder = c(0, 0), include.mean = TRUE)
  ...
  if (dmodel$skewmodel == "pwl") {
    ...
    if (dmodel$skewshocktype == 1) #Skewshocktype ==1 using the squared values, else using the absolute value
      modelinc[23] = 0 else modelinc[23] = 1
  }
  ...
  if (dmodel$shape1model == "pwl") {
    if (dmodel$shape1shocktype == 1)
      modelinc[24] = 2 else modelinc[24] = 3
  } else {
    if (dmodel$shape1shocktype == 1)
      modelinc[24] = 0 else modelinc[24] = 1
  }
  if (dmodel$shape2model == "pwl") {
    if (dmodel$shape2shocktype == 1)
      modelinc[25] = 2 else modelinc[25] = 3
  } else {
    if (dmodel$shape2shocktype == 1)
      modelinc[25] = 0 else modelinc[25] = 1
  }
  ...
  if (is.null(dmodel$skewOrder)) { # If we do not have time-varying skew, then only calculate the unconditional skew
    modelinc[11:14] = 0
    modelinc[8] = di$include.skew
    sbounds[1] = di$skew.LB
    sbounds[2] = di$skew.UB
    } else {
        modelinc[11] = 1
        modelinc[12] = dmodel$skewOrder[1]
        modelinc[13] = dmodel$skewOrder[2]
        modelinc[14] = dmodel$skewOrder[3]
        modelinc[8] = 0
        sbounds[1] = di$skew.LB
        sbounds[2] = di$skew.UB
      }
if(dmodel$model == "sgt"){
  if (is.null(dmodel$shape1Order)) {
    modelinc[15:18] = 0
    modelinc[9] = di$include.shape1
    sbounds[3] = di$shape1.LB
    sbounds[4] = di$shape1.UB
  } else {
    modelinc[15] = 1
    modelinc[16] = dmodel$shape1Order[1]
    modelinc[17] = dmodel$shape1Order[2]
    modelinc[18] = dmodel$shape1Order[3]
    modelinc[9] = 0
    sbounds[3] = di$shape1.LB
    sbounds[4] = di$shape1.UB
  }
  if (is.null(dmodel$shape2Order)) {
    modelinc[19:22] = 0
    modelinc[10] = di$include.shape2
    sbounds[5] = di$shape2.LB
    sbounds[6] = di$shape2.UB
  } else {
    modelinc[19] = 1
    modelinc[20] = dmodel$shape2Order[1]
    modelinc[21] = dmodel$shape2Order[2]
    modelinc[22] = dmodel$shape2Order[3]
    modelinc[10] = 0
    sbounds[5] = di$shape2.LB
    sbounds[6] = di$shape2.UB
  }
...
if(dmodel$model == "sst"){
  ...
  #------
  modelinc[31] = modeldesc$distno
  # 1 = sgt  ; 2 = sged; 3 = sst;
  maxOrder = 1
  modelnames = .expand.model(modelinc)

  pos = 1
  pos.matrix = matrix(0, ncol = 3, nrow = 22)
  colnames(pos.matrix) = c("start", "stop", "include")
  rownames(pos.matrix) = c("mu", "ar", "ma", "omega", "alpha", "beta", "gamma",
                           "skew", "shape1","shape2",
                           "skcons", "skalpha", "skgamma", "skbeta",
                           "sh1cons", "sh1alpha", "sh1gamma", "sh1beta",
                           "sh2cons", "sh2alpha", "sh2gamma", "sh2beta")
  ...
  # Parameter Matrix
  #------
  mm = sum(modelinc[c(2, 3, 5, 6, 7, 12, 13, 14, 16, 17, 18,20,21,22)])
  mm = mm - length(which(modelinc[c(2, 3, 5, 6, 7, 12, 13, 14, 16, 17, 18,20,21,22)] > 0))
  pars = matrix(0, ncol = 6, nrow = 22 + mm)
  colnames(pars) = c("Level", "Fixed", "Include", "Estimate", "LB", "UB")
  pidx = matrix(NA, nrow = 22, ncol = 2)
  colnames(pidx) = c("begin", "end")
  rownames(pidx) = c("mu", "ar", "ma", "omega", "alpha", "beta", "gamma",
                     "skew", "shape1","shape2",
                     "skcons", "skalpha", "skgamma", "skbeta",
                     "sh1cons", "sh1alpha", "sh1gamma", "sh1beta",
                     "sh2cons", "sh2alpha", "sh2gamma", "sh2beta")
