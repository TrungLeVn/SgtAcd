...
{
  int k = 0; /* currently only allowing for maxlag = 1 in both skew and shape equations */
	if( model[13]>0 )
	{
	  tempskew[i] += skewdynamicsT(model,idx,pars,z[i-1],sqrt(fabs(h[i-1])),tempskew[i-1],k);
		...
		double *sbounds, double *h, int i, int T)
{
  int k = 0;
	if( model[18]>0 )
	{
   tempshape1[i] += shape1dynamics(model,idx,pars,z[i-1],sqrt(fabs(h[i-1])),tempshape1[i-1],k);
                     ...
                     double *sbounds, double *h, int i, int T)
{
  ...
  if( model[23]>0 )
  {
    tempshape2[i] += shape2dynamics(model, idx, pars, z[i-1],sqrt(fabs(h[i-1])),tempshape2[i-1],k);
...
double skewdynamicsT(const int *model, const int *idx, const double *pars, const double z, const double hm, const double lagval, const int lag)
{
	double res=0.0;
	switch(model[28])
	{
		case 0:
		{
			...
			if(model[17]>lag) res+=pars[idx[17]]*hm;
 			break;
		}
		...
		case 1:
		{
			...
			if(model[16]>lag) res+=pars[idx[16]+lag]*lagval;
			if(model[17]>lag) res+=pars[idx[17]+lag]*hm;
			break;
		}
	case 2:
		{
...
double shape1dynamics(const int *model, const int *idx, const double *pars, const double z, const double hm, const double lagval, const int lag)
{
	double res=0.0;
	switch(model[29])
	{
	  ...
	  // quadric model with squared
		case 0:
		{
			...
			if(model[22]>lag) res+=pars[idx[22]+lag]*hm;
			break;
		}
...
double shape2dynamics(const int *model, const int *idx, const double *pars, const double z, const double hm, const double lagval, const int lag)
{
  double res=0.0;
  switch(model[30])
  {
  ...
  case 0:
  {
    ...
    if(model[27]>lag) res+=pars[idx[27]+lag]*hm;
    break;
  }
  ...
  case 3:
  {
    ...
    if(model[25]>lag) res+=pars[idx[25]+lag]*z2;
    if(model[26]>lag) res+=pars[idx[26]+lag]*lagval;
    if(model[27]>lag) res+=pars[idx[27]+lag]*lagval;
    break;
  }
  }
  return res;
